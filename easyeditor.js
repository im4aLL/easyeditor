function EasyEditor(options){
    ...
    this.buttons = ['bold', 'italic', 'h2', 'h3', 'h4', 'x', 'alignleft', 'aligncenter', 'alignright', 'quote', 'code', 'link', 'image'];
...
EasyEditor.prototype.handleResizeImage = function(){
    ...
    $(_this.elem).delegate('figure.is-resizable', 'mousemove', function(event) {
        $(this).find('img').css({ 'width' : $(_this.elem).find('figure').width() + 'px' });
    });
...
EasyEditor.prototype.utils = function(){
    var _this = this;
    $('.'+ _this.className +'-modal-close').click(function(event) {
...
EasyEditor.prototype.closeModal = function(selector){
    ...
    $(this.elem).find('temp').contents().unwrap();
};
